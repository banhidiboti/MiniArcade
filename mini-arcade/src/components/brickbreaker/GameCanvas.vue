...
const drawHUD = () => {
  ...
  ctx.fillStyle = 'rgba(0, 240, 255, 0.55)'
  const livesValue = Array.from({ length: 3 }, (_, index) => (index < lives.value ? '♥' : '♡')).join(' ')
  const livesValueWidth = ctx.measureText('♥ ♥ ♥').width
  const rightPadding = showHudPause.value ? 108 : 16
  ctx.fillText('LIVES: ', WIDTH - rightPadding - livesValueWidth, midY)
  ctx.fillStyle = '#00f0ff'
  ctx.fillText(livesValue, WIDTH - rightPadding, midY)
